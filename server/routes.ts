...
COMMIT;
  ...
  app.get('/api/maintenance/requests/machine/:machineId', async (req, res) => {
    try {
  ...
  app.get('/api/maintenance/requests/status/:status', async (req, res) => {
    try {
  ...
  app.get('/api/maintenance/requests/user/:userId', async (req, res) => {
    try {
      const { userId } = req.params;
      const requests = await storage.getMaintenanceRequestsByUser(userId);
      res.json(requests);
    } catch (error) {
      console.error(`Error fetching maintenance requests by user ${req.params.userId}:`, error);
      res.status(500).send("Internal server error");
    }
  });
  ...
  app.get('/api/maintenance/requests/:id', async (req, res) => {
    try {
      ...
      console.error(`Error fetching maintenance request ${req.params.id}:`, error);
      res.status(500).send("Internal server error");
    }
  });
  ...
  app.post('/api/maintenance/requests', async (req, res) => {
    try {
      ...
      // Generate a unique request number with date prefix
      const datePrefix = new Date().toISOString().slice(0, 10).replace(/-/g, '');
      const randomSuffix = Math.floor(1000 + Math.random() * 9000); // 4-digit random number
      requestData.requestNumber = `MR-${datePrefix}-${randomSuffix}`;
      
      // Set the requestedBy field to the current user
      requestData.requestedBy = req.user.username;
  ...
  app.put('/api/maintenance/requests/:id', async (req, res) => {
    try {
      ...
      const updatedRequest = await storage.updateMaintenanceRequest(id, req.body);
      res.json(updatedRequest);
    } catch (error) {
      console.error(`Error updating maintenance request ${req.params.id}:`, error);
      res.status(400).send("Bad request");
    }
  });
  ...
  app.delete('/api/maintenance/requests/:id', async (req, res) => {
    try {
      ...
      if (success) {
        ...
        res.status(500).send("Failed to delete maintenance request");
      }
    } catch (error) {
      console.error(`Error deleting maintenance request ${req.params.id}:`, error);
      res.status(500).send("Internal server error");
    }
  });
  ...
  app.get('/api/maintenance/actions/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const action = await storage.getMaintenanceAction(id);
      if (!action) {
        return res.status(404).send("Maintenance action not found");
      }
      res.json(action);
    } catch (error) {
      console.error(`Error fetching maintenance action ${req.params.id}:`, error);
      res.status(500).send("Internal server error");
    }
  });
  ...
  app.post('/api/maintenance/actions', async (req, res) => {
    try {
      ...
      // Set the performedBy field to the current user
      actionData.performedBy = req.user.username;
      
      const newAction = await storage.createMaintenanceAction(actionData);
      
      // If this is a completion action, update the maintenance request status
      if (actionData.status === "completed" && actionData.requestId) {
        await storage.updateMaintenanceRequest(actionData.requestId, {
          status: "fixed",
  ...
  app.put('/api/maintenance/actions/:id', async (req, res) => {
    try {
      ...
      const updatedAction = await storage.updateMaintenanceAction(id, req.body);
      res.json(updatedAction);
    } catch (error) {
      console.error(`Error updating maintenance action ${req.params.id}:`, error);
      res.status(400).send("Bad request");
    }
  });
  ...
  app.delete('/api/maintenance/actions/:id', async (req, res) => {
    try {
      ...
      if (success) {
        ...
        res.status(500).send("Failed to delete maintenance action");
      }
    } catch (error) {
      console.error(`Error deleting maintenance action ${req.params.id}:`, error);
      res.status(500).send("Internal server error");
    }
  });

  // Spare Parts
  app.get('/api/maintenance/spare-parts', async (req, res) => {
    try {
  ...
  app.get('/api/maintenance/spare-parts/low-stock', async (req, res) => {
    try {
      const parts = await storage.getLowStockSpareParts();
      res.json(parts);
    } catch (error) {
  ...
  app.get('/api/maintenance/spare-parts/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      console.error(`Error fetching spare part ${req.params.id}:`, error);
      res.status(500).send("Internal server error");
    }
  });

  app.post('/api/maintenance/spare-parts', async (req, res) => {
    try {
      const partData = req.body;
      const newPart = await storage.createSparePart(partData);
  ...
  app.put('/api/maintenance/spare-parts/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const part = await storage.getSparePart(id);
      if (!part) {
      ...
      const updatedPart = await storage.updateSparePart(id, req.body);
      res.json(updatedPart);
    } catch (error) {
      console.error(`Error updating spare part ${req.params.id}:`, error);
      res.status(400).send("Bad request");
    }
  });

  app.delete('/api/maintenance/spare-parts/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const part = await storage.getSparePart(id);
      if (!part) {
      ...
      const success = await storage.deleteSparePart(id);
      if (success) {
        res.status(204).send();
      } else {
        res.status(500).send("Failed to delete spare part");
      }
    } catch (error) {
      console.error(`Error deleting spare part ${req.params.id}:`, error);
      res.status(500).send("Internal server error");
    }
  });

  // Maintenance Logbook
  app.get('/api/maintenance/logbook', async (req, res) => {
    try {
      const entries = await storage.getMaintenanceLogbook();
      res.json(entries);
    } catch (error) {
      console.error("Error fetching maintenance logbook entries:", error);
      res.status(500).send("Internal server error");
    }
  });

  app.get('/api/maintenance/logbook/machine/:machineId', async (req, res) => {
    try {
      const { machineId } = req.params;
      const entries = await storage.getMaintenanceLogbookByMachine(machineId);
      res.json(entries);
    } catch (error) {
      console.error(`Error fetching maintenance logbook entries for machine ${req.params.machineId}:`, error);
      res.status(500).send("Internal server error");
    }
  });

  app.get('/api/maintenance/logbook/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const entry = await storage.getMaintenanceLogbookEntry(id);
      if (!entry) {
        return res.status(404).send("Maintenance logbook entry not found");
      }
      res.json(entry);
    } catch (error) {
      console.error(`Error fetching maintenance logbook entry ${req.params.id}:`, error);
      res.status(500).send("Internal server error");
    }
  });

  app.post('/api/maintenance/logbook', async (req, res) => {
    try {
      if (!req.user) {
        return res.status(401).send("Unauthorized");
      }
      ...
      entryData.createdBy = req.user.username;
      
      const newEntry = await storage.createMaintenanceLogbookEntry(entryData);
      res.status(201).json(newEntry);
    } catch (error) {
      console.error("Error creating maintenance logbook entry:", error);
      res.status(400).send("Bad request");
    }
  });

  app.put('/api/maintenance/logbook/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const entry = await storage.getMaintenanceLogbookEntry(id);
      if (!entry) {
      ...
      const updatedEntry = await storage.updateMaintenanceLogbookEntry(id, req.body);
      res.json(updatedEntry);
    } catch (error) {
      console.error(`Error updating maintenance logbook entry ${req.params.id}:`, error);
      res.status(400).send("Bad request");
    }
  });

  app.delete('/api/maintenance/logbook/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const entry = await storage.getMaintenanceLogbookEntry(id);
      if (!entry) {
      ...
      const success = await storage.deleteMaintenanceLogbookEntry(id);
      if (success) {
        res.status(204).send();
      } else {
        res.status(500).send("Failed to delete maintenance logbook entry");
      }
    } catch (error) {
      console.error(`Error deleting maintenance logbook entry ${req.params.id}:`, error);
      res.status(500).send("Internal server error");
    }
  });
  ...
  app.get('/api/maintenance/schedule', async (req, res) => {
    try {
      const tasks = await storage.getMaintenanceSchedule();
      res.json(tasks);
    } catch (error) {
      console.error("Error fetching maintenance schedule tasks:", error);
      res.status(500).send("Internal server error");
    }
  });

  app.get('/api/maintenance/schedule/overdue', async (req, res) => {
    try {
      const tasks = await storage.getOverdueMaintenance();
      res.json(tasks);
    } catch (error) {
  ...
  app.get('/api/maintenance/schedule/upcoming/:days', async (req, res) => {
    try {
      const days = parseInt(req.params.days);
      const tasks = await storage.getUpcomingMaintenance(days);
      res.json(tasks);
    } catch (error) {
      console.error(`Error fetching upcoming maintenance tasks for next ${req.params.days} days:`, error);
      res.status(500).send("Internal server error");
    }
  });

  app.get('/api/maintenance/schedule/:id', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const task = await storage.getMaintenanceScheduleTask(id);
      if (!task) {
        return res.status(404).send("Maintenance schedule task not found");
      }
      res.json(task);
    } catch (error) {
      console.error(`Error fetching maintenance schedule task ${req.params.id}:`, error);
      res.status(500).send("Internal server error");
    }
  });
  ...
  app.post('/api/maintenance/schedule', async (req, res) => {
    try {
      ...
      // Set the createdBy field to the current user
      taskData.createdBy = req.user.username;
      
      const newTask = await storage.createMaintenanceScheduleTask(taskData);
      ...
      switch(taskData.frequency) {
        ...
        case 'annually':
          ...
          break;
        default:
          nextDue.setDate(today.getDate() + 30); // Default to 30 days
      }
      
      await storage.updateMaintenanceScheduleTask(newTask.id, { nextDue });
      
      // Get the updated task with the next due date
      const updatedTask = await storage.getMaintenanceScheduleTask(newTask.id);
      res.status(201).json(updatedTask);
    } catch (error) {
      console.error("Error creating maintenance schedule task:", error);
      res.status(400).send("Bad request");
    }
  });
  ...
  app.put('/api/maintenance/schedule/:id', async (req, res) => {
    try {
      ...
      const updatedTask = await storage.updateMaintenanceScheduleTask(id, req.body);
      res.json(updatedTask);
    } catch (error) {
      console.error(`Error updating maintenance schedule task ${req.params.id}:`, error);
      res.status(400).send("Bad request");
    }
  });
  ...
  app.delete('/api/maintenance/schedule/:id', async (req, res) => {
    try {
      ...
      const success = await storage.deleteMaintenanceScheduleTask(id);
      if (success) {
        res.status(204).send();
      } else {
        res.status(500).send("Failed to delete maintenance schedule task");
      }
    } catch (error) {
      console.error(`Error deleting maintenance schedule task ${req.params.id}:`, error);
      res.status(500).send("Internal server error");
    }
  });
