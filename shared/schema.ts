...
// Quality Violation table
export const qualityViolations = pgTable("quality_violations", {
  id: serial("id").primaryKey(),
  qualityCheckId: integer("quality_check_id").references(() => qualityChecks.id),
  reportDate: timestamp("report_date").defaultNow().notNull(),
  reportedBy: text("reported_by").references(() => users.id),
  severity: text("severity").notNull(), // low, medium, high, critical
  description: text("description").notNull(),
  impact: text("impact"),
  status: text("status").notNull().default("reported"), // reported, investigating, resolved, dismissed
...
// Quality Penalties table
export const qualityPenalties = pgTable("quality_penalties", {
  id: serial("id").primaryKey(),
  violationId: integer("violation_id").notNull().references(() => qualityViolations.id),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  description: text("description").notNull(),
  penaltyType: text("penalty_type").notNull(), // warning, training, suspension, financial, other
